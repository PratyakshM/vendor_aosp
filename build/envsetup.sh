...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Atom-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop org.atom.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.atom.device | grep -q "$LINEAGE_BUILD");
    then
